  ...
  return (
    <>
      ...
      <div className="bg-first w-full p-4">
        <h2 className="text-xl sm:text-2xl md:text-3xl mb-10 text-center text-fourth scp">Skills</h2>
        <div className="flex justify-center">
          <div className="flex flex-wrap justify-around gap-2 sm:gap-4"> {/* Adjusted for wrapping and spacing */}
            {sortedSkills.map(skill => (
              <div key={skill.name} className="text-center w-20 sm:w-24 md:w-28 mb-4"> {/* Added margin-bottom for spacing between rows */}
                {/* Outer div representing max height */}
                <div className="h-[96px] w-8 sm:w-10 md:w-12 bg-third relative mx-auto rounded">
                  {/* Inner div representing skill value */}
                  <div
                    className="absolute bottom-0 bg-second transition-all duration-500 ease-in-out w-full rounded"
                    style={{ height: `${Math.min(skill.value, 100) * 0.96}px` }}
                  ></div>
                </div>
                <p className="mt-2 text-xs sm:text-sm md:text-base text-center truncate text-fourth scp">{skill.name}</p>
              </div>
            ))}
          </div>
        </div>
